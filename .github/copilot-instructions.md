...
- `PluginContext` exposes: `register_tool(cls)`, `register_tts(name, factory)`, `register_stt(name, factory)`, `register_chatbox_source(name, source, priority)`, `subscribe(event, cb)`, `plugin_config(key)`, `data_dir()`, lazy `audio` / `osc` / `session` / `tool_handler` refs.
